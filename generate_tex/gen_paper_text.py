...
from generate_tex import get_keys
...
def gen_paper_text(user_prompt: str, client: Anthropic | None = None):
# user_prompt = "LLM Safety"

    if client is None:
        client = Anthropic(
            api_key=get_keys.get_anthropic_key()
        )
    ...
    result_output = get_articles(user_prompt)
    result_output[0]
    ...
    tex_file_name = f"{user_prompt[:12]}_{str(hash(user_prompt))[-8:]}.tex"
    with open(tex_file_name, "w") as f:
        f.write(latex_content)

    print(f"LaTeX file '{tex_file_name}' has been generated.")
